...
```bash
git clone https://github.com/styrene-lab/omegon-pi.git
cd omegon-pi
./scripts/install-pi.sh   # npm install + npm link
...
```bash
# 1. Edit extensions, skills, themes, etc.
# 2. Restart omegon-pi — changes are live (no build step needed for extensions)
```
