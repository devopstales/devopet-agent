> **OpenSpec change:** `ai-provider-extensions` (provider auth and extended model sources).  
> **Implementation:** first-party **`extensions/ai-provider-connect/`** (must load before `security-engine`). **[pi-connect](https://www.npmjs.com/package/pi-connect)** is a **behavioral reference**, not a mandatory npm dependency.
