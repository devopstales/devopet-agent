...
devopet takes **no position** on language, framework, or specific library choices. The directive is: if the user wants runtime enforcement, the contract must be loaded from the spec file — not reconstructed from code annotations.
...
Every project using devopet has a `.pi/memory/facts.jsonl` file that is git-tracked and uses `merge=union` in `.gitattributes`. These rules apply to ALL repositories:
