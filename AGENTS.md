# devopet Project Directives

> Global directives (attribution, completion standards, memory sync, branch hygiene) are deployed automatically to `~/.pi/agent/AGENTS.md` by the defaults extension on first session start. They apply to all projects. To customize, edit that file — but remove the `<!-- managed by devopet -->` marker or your changes will be overwritten on update.
...
Key points for working on devopet itself:
