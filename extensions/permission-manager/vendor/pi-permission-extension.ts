...
import { toRecord } from "../../../node_modules/pi-permission-system/src/common.js";
import {
  DEFAULT_EXTENSION_CONFIG,
  loadPermissionSystemConfig,
  type PermissionSystemExtensionConfig,
} from "../../../node_modules/pi-permission-system/src/extension-config.js";
import { createPermissionSystemLogger } from "../../../node_modules/pi-permission-system/src/logging.js";
import { PermissionManager } from "../../../node_modules/pi-permission-system/src/permission-manager.js";
import { sanitizeAvailableToolsSection } from "../../../node_modules/pi-permission-system/src/system-prompt-sanitizer.js";
import { checkRequestedToolRegistration, getToolNameFromValue } from "../../../node_modules/pi-permission-system/src/tool-registry.js";
import type { PermissionCheckResult, PermissionState } from "../../../node_modules/pi-permission-system/src/types.js";
import {
  appendToolPathRule,
  DEFAULT_ASK_POLICY,
  parsePolicyDocument,
  persistEffectivePolicy,
  resolveDevopetPermissionPolicy,
} from "../policy.ts";
import {
  extractToolInputPath,
  matchToolPathPermission,
  PATH_SCOPED_TOOLS,
  suggestPathGlob,
} from "../tool-path-policy.ts";
import { isYoloDestructiveBash } from "../yolo-destructive-bash.ts";
...
function formatAskPrompt(
  ...
  agentName?: string,
  pathInfo?: { absolutePath: string; suggestedGlob: string },
): string {
  ...
  if (pathInfo && PATH_SCOPED_TOOLS.has(result.toolName)) {
    const patternInfo = result.matchedPattern ? ` (policy matched '${result.matchedPattern}')` : "";
    return `${subject} requested ${result.toolName} to '${pathInfo.absolutePath}' (persisted allow rules use glob '${pathInfo.suggestedGlob}')${patternInfo}. Allow this?`;
  }
...
/** Session-scoped allow keys (pi-sandbox–style elevation). */
const sessionElevationKeys = new Set<string>();

function makeGateKey(parts: Record<string, string | undefined>): string {
  return JSON.stringify(parts);
}

function readMergedToolPaths(cwd: string) {
  const r = resolveDevopetPermissionPolicy(cwd);
  if (r.kind !== "policy") {
    return {};
  }
  const raw = readFileSync(r.mergedPath, "utf8");
  return parsePolicyDocument(r.mergedPath, raw).config.toolPaths ?? {};
}
...
async function confirmPermission(
  ...
  pathPersist: { toolName: string; glob: string } | undefined,
  onAfterPolicyWrite: () => void,
): Promise<boolean> {
  if (ctx.hasUI) {
    const choices = pathPersist
      ? [
          "Allow once",
      ...
      : ["Allow once", "Allow for this session", "Abort"];
    const choice = await ctx.ui.select(`Permission required — ${message}`, choices);
    ...
    if (choice === "Allow for this session") {
      if (gateKey) {
        sessionElevationKeys.add(gateKey);
      }
      return true;
    }
    if (choice === "Allow for this repository" && pathPersist) {
      try {
        appendToolPathRule("project", ctx.cwd ?? process.cwd(), pathPersist.toolName, pathPersist.glob, "allow");
        onAfterPolicyWrite();
        ctx.ui.notify(`Saved allow for '${pathPersist.glob}' to .devopet/permissions.jsonc`, "info");
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        ctx.ui.notify(`Could not save project permissions: ${msg}`, "error");
        return false;
      }
      return true;
    }
    if (choice === "Allow globally" && pathPersist) {
      try {
        appendToolPathRule("global", ctx.cwd ?? process.cwd(), pathPersist.toolName, pathPersist.glob, "allow");
        onAfterPolicyWrite();
        ctx.ui.notify(`Saved allow for '${pathPersist.glob}' to ~/.devopet/permissions.jsonc`, "info");
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        ctx.ui.notify(`Could not save global permissions: ${msg}`, "error");
        return false;
      }
      return true;
    }
...
export default function piPermissionSystemExtension(pi: ExtensionAPI): void {
  /** When true, user set `yolo: true` in permissions JSONC — policy hooks no-op. */
  let devopetYolo = false;
  let permissionManager = new PermissionManager({
    globalConfigPath: join(PI_AGENT_DIR, "pi-permissions.jsonc"),
  });
  ...
  const applyDevopetResolution = (ctx: ExtensionContext): void => {
    const r = resolveDevopetPermissionPolicy(ctx.cwd);
    if (r.kind === "error") {
      devopetYolo = false;
      if (ctx.hasUI) {
        ctx.ui.notify(`Permission policy (${r.path}): ${r.message}`, "error");
        ctx.ui.setStatus("permission-manager", "ERR");
      }
      const fallbackPath = persistEffectivePolicy(DEFAULT_ASK_POLICY);
      permissionManager = new PermissionManager({ globalConfigPath: fallbackPath });
      return;
    }
    if (r.yolo) {
      devopetYolo = true;
      if (ctx.hasUI) {
        ctx.ui.setStatus("permission-manager", "YOLO");
      }
    ...
    devopetYolo = false;
    if (ctx.hasUI) {
      const label = r.source === "default-ask" ? "ask-default" : "policy";
      ctx.ui.setStatus("permission-manager", label);
    }
    permissionManager = new PermissionManager({ globalConfigPath: r.mergedPath });
  };
  ...
  const promptPermission = async (
    ...
    details: {
      ...
      command?: string;
      target?: string;
      /** When set, menu includes “repository” / “global” persist options (path-scoped tools). */
      pathPersist?: { toolName: string; glob: string };
    },
  ): Promise<boolean> => {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    if (devopetYolo) {
      ...
      for (const tool of allTools) {
        const tn = getEventToolName(tool);
        if (tn) {
          allowedTools.push(tn);
        }
      }
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (devopetYolo) {
      if (toolName === "bash") {
        const inputEarly = getEventInput(event);
        const recEarly = toRecord(inputEarly);
        const cmdEarly = typeof recEarly.command === "string" ? recEarly.command : "";
        if (cmdEarly && isYoloDestructiveBash(cmdEarly)) {
          if (!toolName) {
            return { block: true, reason: formatMissingToolNameReason() };
          }
          const regEarly = checkRequestedToolRegistration(toolName, pi.getAllTools());
          if (regEarly.status === "missing-tool-name") {
            return { block: true, reason: formatMissingToolNameReason() };
          }
          if (regEarly.status === "unregistered") {
            return {
              block: true,
              reason: formatUnknownToolReason(regEarly.requestedToolName, regEarly.availableToolNames),
            };
          }
          if (!canRequestPermissionConfirmation(ctx)) {
            return {
              block: true,
              reason:
                "Destructive shell command requires approval, but no interactive UI is available (YOLO safety gate).",
            };
          }
          const yoloGateKey = makeGateKey({
            source: "tool_call",
            toolName: "bash",
            skillName: "",
            path: "",
          ...
          if (sessionElevationKeys.has(yoloGateKey)) {
            return {};
          }
          const yoloMsg = `YOLO mode (yolo: true in permissions.jsonc): allow destructive shell command?\n${cmdEarly}`;
          const yoloApproved = await promptPermission(ctx, {
          ...
          if (!yoloApproved) {
            return {
              block: true,
              reason: "User denied destructive shell command (YOLO safety gate).",
            };
          }
          return {};
        }
    ...
    if (!devopetYolo && toolName && PATH_SCOPED_TOOLS.has(toolName)) {
      const abs = extractToolInputPath(input, cwd);
      if (abs) {
        const toolPaths = readMergedToolPaths(cwd);
        const pathDecision = matchToolPathPermission(toolPaths, toolName, abs);
        ...
        if (pathDecision === "deny") {
          writeReviewLog("permission_request.blocked", {
            ...
            agentName,
            path: abs,
            resolution: "tool_path_policy_denied",
          });
          return {
            block: true,
            reason: `Path policy denies ${toolName} to '${abs}'.`,
          };
        }
      }
    }
    ...
    if (check.state === "ask") {
      const resolvedPath =
        toolName && PATH_SCOPED_TOOLS.has(toolName) ? extractToolInputPath(input, cwd) : null;
      const pathForGate = resolvedPath ?? "";
      const elevationKey = makeGateKey({
        source: "tool_call",
        toolName,
        skillName: "",
        path: pathForGate,
      ...
      if (sessionElevationKeys.has(elevationKey)) {
        return {};
      }
      ...
      let pathInfo: { absolutePath: string; suggestedGlob: string } | undefined;
      let pathPersist: { toolName: string; glob: string } | undefined;
      if (resolvedPath && toolName && PATH_SCOPED_TOOLS.has(toolName)) {
        const sg = suggestPathGlob(resolvedPath);
        pathInfo = { absolutePath: resolvedPath, suggestedGlob: sg };
        pathPersist = { toolName, glob: sg };
      }

      const message = formatAskPrompt(check, agentName ?? undefined, pathInfo);
      if (!canRequestPermissionConfirmation(ctx)) {
        writeReviewLog("permission_request.blocked", {
          source: "tool_call",
          toolCallId: event.toolCallId,
